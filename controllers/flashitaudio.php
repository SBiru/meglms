...
if(isset($_SESSION['USER']) && isset($_SESSION['USER']['LOGGED_IN']) && isset($_SESSION['USER']['ID']) && $_SESSION['USER']['ID'] > 0 && $_SESSION['USER']['LOGGED_IN']==true) {
	...
	preg_match('/\/flashitaudio\/(?P<vocabID>\d*)$/', $_SERVER['REQUEST_URI'], $matches);
	...
	if($action=='update'){
		...
		$q = "INSERT INTO `flash_it_audio_right_id` (`correct`, `incorrect`, `remaining`, `user_id`, `vocabulary_id`) VALUES( {$correct}, {$incorrect}, {$remaining}, {$user_id}, {$vocabID}) ON DUPLICATE KEY UPDATE correct=VALUES(correct), incorrect=VALUES(incorrect), remaining=VALUES(remaining)";
		$DB->mysqli->query($q);
		$data = new \stdClass();
		$data->success = true;
		//$data->query = $q;
	...
	if($action=='fetch'){
		$q = "SELECT `correct`, `incorrect`, `remaining`  FROM `flash_it_audio_right_id` WHERE `user_id` = {$user_id} and `vocabulary_id` = {$vocabID} limit 1";
