...
appDirectives.directive('autoPlayVideo',function(){
  return function(scope,element){
    element.play();
  }
});
appDirectives.directive('scrollTo', function () {
  return function (scope, element, attributes) {
    setTimeout(function () {
      if (scope.$eval(attributes.scrollTo)) {
        var questionId = attributes.scrollTo;
        var q = _.findWhere(element,{îd:questionId});
        if(q){
          window.scrollTo(0, element[element.indexOf(q)].offsetTop - 100)
        }else{
...
appDirectives.directive( 'popoverHtmlPopup', [ function() {
  return {
    restrict: 'EA',
    replace: true,
...
appDirectives.directive('abnTree', [
...
]).directive('graderDateRange',[function(){
    return {
        require:'ngModel',
        restrict:'AC',
        link:function(scope,el,attrs,ngModel){
            ...
            $(el).on('apply.daterangepicker', function(ev, picker) {
                startDateVal.html(picker.startDate.format('MM/DD/YYYY'));
                endDateVal.html(picker.endDate.format('MM/DD/YYYY'));
                ngModel.$setViewValue({min:picker.startDate.format('MM/DD/YYYY'),max:picker.endDate.format('MM/DD/YYYY')});
                elWrapper.addClass('has-values')
            });

            $(el).on('cancel.daterangepicker', function(ev, picker) {
                startDateVal.html('');
                endDateVal.html('');
                ngModel.$setViewValue({min:null,max:null});
                elWrapper.removeClass('has-values')
            });
            ...
            function prepareTemplate(){
                graderDateRangeId++;
                id = 'grader-date-range'+graderDateRangeId;
                elWrapper = $('<div class="grader-date-range-wrapper"></div>');
                button = $('<div class="btn btn-default" id="'+id+'"><span class="fa' +
                    ' fa-calendar"></span></div>');
                labelWrapper = $('<div class="grader-date-range-values"></div>');
                startDateVal = $('<div></div>');
                endDateVal = $('<div></div>');
                labelWrapper.append(startDateVal);
                labelWrapper.append(endDateVal);
