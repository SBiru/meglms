...
( function() {
	...
	// Dialog reused by both 'CreateScottsDiv' and 'editdiv' commands.
	// @param {Object} editor
	...
	function divDialog( editor, command ) {
		...
		// DTD of 'div' element
		var dtd = CKEDITOR.dtd.scottsdiv;
		...
		// @param {Object} editor
		function CreateScottsDiv( editor ) {
			...
			for ( i = 0; i < blockGroups.length; i++ ) {
				...
				divElement = new CKEDITOR.dom.element( 'scottsdiv', editor.document );
		...
		function commitInternally( targetFields ) {
			var dialog = this.getDialog(),
				element = dialog._element && dialog._element.clone() || new CKEDITOR.dom.element( 'scottsdiv', editor.document );
		...
		// @type divDialog
		return {
			title: editor.lang.scottsdiv.title,
			minWidth: 400,
			minHeight: 165,
			contents: [
				{
				...
				elements: [
					...
					children: [
						...
						type: 'select',
						style: 'width: 100%;',
						label: editor.lang.scottsdiv.styleSelectLabel,
						'default': '',
						...
						id: 'class',
						type: 'text',
						requiredContent: 'scottsdiv(cke-xyz)', // Random text like 'xyz' will check if all are allowed.
				...
				elements: [
					...
					children: [
						...
						children: [
							...
							type: 'text',
							id: 'lang',
							requiredContent: 'scottsdiv[lang]',
						...
						children: [
							...
							type: 'text',
							id: 'title',
							requiredContent: 'scottsdiv[title]',
							style: 'width: 100%;',
				...
				editor.getStylesSet( function( stylesDefinitions ) {
					...
					if ( stylesDefinitions ) {
						...
						for ( var i = 0; i < stylesDefinitions.length; i++ ) {
							...
							if ( styleDefinition.element && styleDefinition.element == 'scottsdiv' ) {
			...
			onShow: function() {
				...
				if ( command == 'editdiv' ) {
					...
					this.setupContent( this._element = CKEDITOR.plugins.scottsdiv.getSurroundDiv( editor ) );
				}
			},
			onOk: function() {
				if ( command == 'editdiv' )
					...
					containers = CreateScottsDiv( editor, true );
	...
	CKEDITOR.dialog.add( 'CreateScottsDiv', function( editor ) {
		return divDialog( editor, 'CreateScottsDiv' );
	} );
