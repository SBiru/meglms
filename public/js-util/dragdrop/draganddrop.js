        ...
        function ($parse, $rootScope, $dragImage) {
            return function (scope, element, attrs) {
                var isDragHandleUsed = false,
                    ...
                    dragTarget;
                var customDragMoveEvent = attrs.useDragMove;
                ...
                customDragMoveEvent = function(e){
                    var container = element.closest('#two');
                    checkContainerPosition(container);

                }
                function checkContainerPosition(container){
                    if(!container.offset()) return;
                    var win = angular.element(window);

                    var currentWinY = win.scrollTop();
                    var currentY = dragY;
                    var winSize = window.innerHeight;
                    var bottom = currentWinY+winSize;
                    var containerBottom = container.offset().top+410;
                    var elementBottom = currentY+element.height();
                    //console.log({
                    ...
                    if(currentY-element.height()/2<container.offset().top){
                        container.scrollTop(container.scrollTop()-10);
                    }
                    else{

                        if(containerBottom+20> bottom){
                            if(elementBottom>bottom){
                                win.scrollTop(win.scrollTop()+10);
                            }
                        }else{
                ...
                if(customDragMoveEvent)
                    element.bind("drag", customDragMoveEvent);
            };
        }
    ])
    .directive("uiOnDrop", [
        ...
        function ($parse, $rootScope) {
            return function (scope, element, attr) {
                ...
                function onDragOver(e) {
                    dragY = e.originalEvent.pageY
                    dragX = e.originalEvent.pageX
                    if (e.preventDefault) {
			...
			var deregisterDragStart = $rootScope.$on("ANGULAR_DRAG_START", function (event, channel) {
                    ...
                    if (isDragChannelAccepted(channel, dropChannel)) {
                        if (attr.dropValidate) {
                            ...
                            var valid = validateFn(scope, {$data: currentData.data, $channel: currentData.channel,$event:event});
                            if (!valid) {
                ...
                scope.$watch(dropChannelChanged, function (value) {
                    if (value) {
                        dropChannel = value;
                    }
                });
                function dropChannelChanged(){
                    return element.attr('drop-channel');
                }
