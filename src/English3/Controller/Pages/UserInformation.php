...
class UserInformation {
    public function load(Request $request,$id){
        $pageData = Utility::getInstance()->fetchRow("SELECT * FROM pages where id = :id",['id'=>$id]);

        $data = UserMetaController::get($_SESSION['USER']['ID']);
        $data['pagename'] = $pageData['name'];
        $data['content'] = $pageData['content'];
        $data['new_post_text'] = $pageData['new_post_text'];
        $classId = ClassesController::_getFromPage(null,$id);
        $data = array_merge($data,ClassesController::_getClassMeta($classId));
        $pageMetaSQL = new PageMetaSQL();
        $pageOptions = $pageMetaSQL->get($id,true);
        $data['pageOptions'] = $pageOptions;
        $data['new_post_text'] = $data['new_post_text']?:'Save';
        $data['new_post_color'] = $data['new_post_color']?:'btn-primary btn-post';
        $data['nationalities'] = Countries::getNationalities();
        $data['languages'] = Countries::getLanguages();
        $data['institutions'] = [['code'=>'1','name'=>'Institution 1']];
        return new JsonResponse($data);
    }
    public function save(Request $request,$id){
        Utility::clearPOSTParams($request);
        $acceptableFields = ['nationality','gender','language','department','department_contact','department_email'];
        foreach ($request->request->all() as $key=>$value){
            if(in_array($key,$acceptableFields)){
                Utility::getInstance()->insert("INSERT INTO user_meta (userid, meta_key, meta_value) values (:userid,:meta_key,:meta_value)
        ...
        $classId = ClassesController::_getFromPage(null,$id);
        $postid = Utility::getInstance()->insert("INSERT INTO posts (classid,userid,pageid,message) values (?,?,?,?)",[
            $classId,$_SESSION['USER']['ID'],$id,''
        ]);
        Utility::getInstance()->reader->update('posts',['postrootparentid'=>$postid,'post_reply_id'=>$postid],['id'=>$postid]);
